
CREATE TABLE "clientdetails" (
  "appId" varchar2(48) NOT NULL,
  "resourceIds" varchar2(256) DEFAULT NULL,
  "appSecret" varchar2(256) DEFAULT NULL,
  "scope" varchar2(256) DEFAULT NULL,
  "grantTypes" varchar2(256) DEFAULT NULL,
  "redirectUrl" varchar2(256) DEFAULT NULL,
  "authorities" varchar2(256) DEFAULT NULL,
  "access_token_validity" number(11) DEFAULT NULL,
  "refresh_token_validity" number(11) DEFAULT NULL,
  "additionalInformation" varchar2(512) DEFAULT NULL,
  "autoApproveScopes" varchar2(256) DEFAULT NULL,
  PRIMARY KEY ("appId")
);

CREATE TABLE "oauth_access_token" (
  "token_id" varchar2(256) DEFAULT NULL,
  "token" blob,
  "authentication_id" varchar2(48) NOT NULL,
  "user_name" varchar2(256) DEFAULT NULL,
  "client_id" varchar2(256) DEFAULT NULL,
  "authentication" blob,
  "refresh_token" varchar2(256) DEFAULT NULL,
  PRIMARY KEY ("authentication_id")
);

CREATE TABLE "oauth_approvals" (
  "userId" varchar2(256) DEFAULT NULL,
  "clientId" varchar2(256) DEFAULT NULL,
  "scope" varchar2(256) DEFAULT NULL,
  "status" varchar2(10) DEFAULT NULL,
  "expiresAt" timestamp  DEFAULT systimestamp,
  "lastModifiedAt" timestamp DEFAULT null
);

CREATE TABLE "oauth_client_details" (
  "client_id" varchar2(48) NOT NULL,
  "resource_ids" varchar2(256) DEFAULT NULL,
  "client_secret" varchar2(256) DEFAULT NULL,
  "scope" varchar2(256) DEFAULT NULL,
  "authorized_grant_types" varchar2(256) DEFAULT NULL,
  "web_server_redirect_uri" varchar2(256) DEFAULT NULL,
  "authorities" varchar(256) DEFAULT NULL,
  "access_token_validity" number(11) DEFAULT NULL,
  "refresh_token_validity" number(11) DEFAULT NULL,
  "additional_information" varchar2(512) DEFAULT NULL,
  "autoapprove" varchar2(256) DEFAULT NULL,
  PRIMARY KEY ("client_id")
);

INSERT INTO "oauth_client_details" VALUES ('app', null, 'app', 'app', 'password,refresh_token', null, null, null, null, null, null);
INSERT INTO "oauth_client_details" VALUES ('webApp', null, 'webApp', 'app', 'authorization_code,password,refresh_token,client_credentials', null, null, null, null, null, null);

CREATE TABLE "oauth_client_token" (
  "token_id" varchar(256) DEFAULT NULL,
  "token" blob,
  "authentication_id" varchar(48) NOT NULL,
  "user_name" varchar(256) DEFAULT NULL,
  "client_id" varchar(256) DEFAULT NULL,
  PRIMARY KEY ("authentication_id")
);

CREATE TABLE "oauth_code" (
  "code" varchar(256) DEFAULT NULL,
  "authentication" blob
);

CREATE TABLE "oauth_refresh_token" (
  "token_id" varchar(256) DEFAULT NULL,
  "token" blob,
  "authentication" blob
);

INSERT INTO "oauth_refresh_token" VALUES ('b96e057b4e1c4920428e833db48d4c15', , );


CREATE TABLE "rc_menu" (
  "id" varchar2(64) NOT NULL,
  "code" varchar2(255) DEFAULT NULL ,
  "p_code" varchar2(255) DEFAULT NULL ,
  "p_id" varchar2(255) DEFAULT NULL ,
  "name" varchar2(255) DEFAULT NULL  ,
  "url" varchar2(255) DEFAULT NULL  ,
  "is_menu" number(11) DEFAULT NULL  ,
  "level" number(11) DEFAULT NULL ,
  "sort" number(11) DEFAULT NULL  ,
  "status" number(11) DEFAULT NULL,
  "icon" varchar2(255) DEFAULT NULL,
  "create_time" date DEFAULT NULL,
  "update_time" date DEFAULT NULL,
  PRIMARY KEY ("id"),
  constraint "FK_CODE" UNIQUE("code")

);

comment on column   "rc_menu"."code" is '菜单编码';
comment on column    "rc_menu"."p_code" is '菜单父编码';
comment on column    "rc_menu"."p_id" is '父菜单ID';
comment on column    "rc_menu"."name" is '名称';
comment on column    "rc_menu"."url" is '请求地址';
comment on column    "rc_menu"."is_menu" is '是否是菜单';
comment on column    "rc_menu"."level" is '菜单层级';
comment on column    "rc_menu"."sort" is '菜单排序';


INSERT INTO "rc_menu" VALUES ('000000000000000000', 'root', '0', '0', '系统根目录', '', '1', '0', '1', '1', null, sysdate, null);
INSERT INTO "rc_menu" VALUES ('893287144657780736', 'system', 'root', '000000000000000000', '系统设置', 'system', '1', '1', '10', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('893288715881807872', 'userList', 'system', '893287144657780736', '用户管理', 'user/list', '1', '2', '1', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('893304960282787840', 'user/add', 'userList', '893288715881807872', '用户添加', 'user/add', '0', '3', '1', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894396523532517376', 'user/edit', 'userList', '893288715881807872', '用户修改', 'user/edit', '0', '3', '1', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894473486712438784', 'user/view', 'userList', '893288715881807872', '用户查看', 'user/View', '0', '3', '2', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894473651837992960', 'user/delete', 'userList', '893288715881807872', '用户删除', 'user/delete', '0', '3', '4', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894475142061621248', 'roleList', 'system', '893287144657780736', '角色管理', 'role/list', '1', '2', '2', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894475827880656896', 'role/add', 'roleList', '894475142061621248', '角色添加', 'role/add', '0', '3', '1', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894475985452269568', 'role/edit', 'roleList', '894475142061621248', '角色编辑', 'role/edit', '0', '3', '2', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894476118730473472', 'role/delete', 'roleList', '894475142061621248', '角色删除', 'role/delete', '0', '3', '2', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894476276402749440', 'role/permission', 'roleList', '894475142061621248', '角色配权', 'role/permission', '0', '3', '3', '1', '', sysdate ,null);
INSERT INTO "rc_menu" VALUES ('894476950951690240', 'menu/list', 'system', '893287144657780736', '菜单管理', 'menu/list', '1', '2', '2', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894477107919323136', 'menu/add', 'menu/list', '894476950951690240', '菜单添加', 'menu/add', '0', '3', '1', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894477244926263296', 'menu/edit', 'menu/list', '894476950951690240', '菜单编辑', 'menu/edit', '0', '3', '2', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894477420512411648', 'menu/delete', 'menu/list', '894476950951690240', '菜单删除', 'menu/delete', '0', '3', '2', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894477851082883072', 'apidoc', 'system', '893287144657780736', 'Api文档', 'swagger-ui.html', '1', '2', '9', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894477995903811584', 'database/log', 'system', '893287144657780736', '数据库日志', 'druid', '1', '2', '10', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('894752734459199488', 'companyList', 'root', '000000000000000000', '公司管理', 'companyList', '1', '1', '1', '1', '', sysdate, null);
INSERT INTO "rc_menu" VALUES ('903459378655395840', '/user/modify', 'userList', '893288715881807872', '密码重置', '/user/modify', '1', '3', '2', '1', '', null, null);

CREATE TABLE "rc_privilege" (
  "role_id" number(11) NOT NULL,
  "menu_id" varchar(255) NOT NULL,
  "create_time" date DEFAULT NULL,
  PRIMARY KEY ("role_id","menu_id")
);

INSERT INTO "rc_privilege" VALUES ('6', '893287144657780736', sysdate);
INSERT INTO "rc_privilege" VALUES ('6', '893288715881807872', sysdate);
INSERT INTO "rc_privilege" VALUES ('6', '893304960282787840', sysdate);
INSERT INTO "rc_privilege" VALUES ('6', '894396523532517376', sysdate);
INSERT INTO "rc_privilege" VALUES ('6', '894473486712438784', sysdate);
INSERT INTO "rc_privilege" VALUES ('6', '894473651837992960', sysdate);
INSERT INTO "rc_privilege" VALUES ('6', '894477851082883072', sysdate);
INSERT INTO "rc_privilege" VALUES ('6', '894477995903811584', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '893287144657780736', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '893288715881807872', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '893304960282787840', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894396523532517376', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894473486712438784', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894473651837992960', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894475142061621248', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894475827880656896', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894475985452269568', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894476118730473472', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894476276402749440', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894476950951690240', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894477107919323136', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894477244926263296', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894477420512411648', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894477851082883072', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894477995903811584', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894752734459199488', sysdate);
INSERT INTO "rc_privilege" VALUES ('8', '894769217763540992', sysdate);
INSERT INTO "rc_privilege" VALUES ('17', '893287144657780736', sysdate);
INSERT INTO "rc_privilege" VALUES ('17', '894477851082883072', sysdate);
INSERT INTO "rc_privilege" VALUES ('17', '894477995903811584', sysdate);
INSERT INTO "rc_privilege" VALUES ('17', '894752734459199488', sysdate);


CREATE TABLE "rc_role" (
  "id" number(11) NOT NULL,
  "name" varchar2(255) DEFAULT NULL,
  "value" varchar2(255) DEFAULT NULL,
  "tips" varchar2(255) DEFAULT NULL,
  "create_time" date DEFAULT NULL,
  "update_time" date DEFAULT NULL,
  "status" number(11) NOT NULL,
  PRIMARY KEY ("id"),
  constraint "unique_role_name" UNIQUE("name"),
  constraint "unique_role_value" UNIQUE("value")
);
INSERT INTO "rc_role" VALUES ('1', '管理员', 'admin', null, sysdate, null, '1');
INSERT INTO "rc_role" VALUES ('2', '超级管理员', 'super', null, sysdate, null, '1');
INSERT INTO "rc_role" VALUES ('3', '用户', 'user', null, sysdate, null, '1');

CREATE TABLE "rc_user" (
  "id" number(11) NOT NULL,
  "avatar" varchar2(255) DEFAULT NULL,
  "username" varchar2(45) DEFAULT NULL,
  "password" varchar2(96) DEFAULT NULL,
  "salt" varchar2(45) DEFAULT NULL,
  "name" varchar2(45) DEFAULT NULL,
  "birthday" date DEFAULT NULL,
  "sex" number(11) DEFAULT NULL,
  "email" varchar2(45) DEFAULT NULL,
  "phone" varchar2(45) DEFAULT NULL,
  "status" number(11) DEFAULT NULL,
  "create_time" date DEFAULT NULL,
  "update_time" date DEFAULT NULL,
  PRIMARY KEY ("id"),
    constraint "unique_user_username" UNIQUE("username")

);
INSERT INTO "rc_user" VALUES ('46', null, 'super', '$2a$10$cKRbR9IJktfmKmf/wShyo.5.J8IxO/7YVn8twuWFtvPgruAF8gtKq', null, '超级管理员', null, '1', null, null, '1', sysdate, null);
INSERT INTO "rc_user" VALUES ('48', null, 'admin', '$2a$10$cKRbR9IJktfmKmf/wShyo.5.J8IxO/7YVn8twuWFtvPgruAF8gtKq', null, '管理员', null, '1', null, null, '1', sysdate, null);
INSERT INTO "rc_user" VALUES ('49', null, 'yangxiufeng', '$2a$10$cKRbR9IJktfmKmf/wShyo.5.J8IxO/7YVn8twuWFtvPgruAF8gtKq', null, '秀秀1', null, '1', null, null, '1', sysdate, null);
INSERT INTO "rc_user" VALUES ('50', null, 'test1', '$2a$10$cKRbR9IJktfmKmf/wShyo.5.J8IxO/7YVn8twuWFtvPgruAF8gtKq', null, 'test1', null, '1', null, null, '1', sysdate, null);
INSERT INTO "rc_user" VALUES ('51', null, 'test2', '$2a$10$cKRbR9IJktfmKmf/wShyo.5.J8IxO/7YVn8twuWFtvPgruAF8gtKq', null, 'test2', null, '1', null, null, '1', sysdate, null);

CREATE TABLE "rc_user_role" (
  "id" number(11) NOT NULL,
  "user_id" number(11) DEFAULT NULL,
  "role_id" number(11) DEFAULT NULL,
  "create_time" date DEFAULT NULL,
  "create_by" varchar2(255) DEFAULT NULL,
  PRIMARY KEY ("id")
);

INSERT INTO "rc_user_role" VALUES ('1', '46', '8', sysdate, null);
INSERT INTO "rc_user_role" VALUES ('2', '48', '6', sysdate, null);
INSERT INTO "rc_user_role" VALUES ('3', '49', '17', sysdate, null);
INSERT INTO "rc_user_role" VALUES ('19', '50', '6', sysdate, '超级管理员');
INSERT INTO "rc_user_role" VALUES ('20', '50', '17', sysdate, '超级管理员');
INSERT INTO "rc_user_role" VALUES ('22', '57', '8', sysdate, '超级管理员');
INSERT INTO "rc_user_role" VALUES ('23', '57', '17', sysdate, '超级管理员');
