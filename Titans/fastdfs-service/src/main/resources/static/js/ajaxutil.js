$(function() {
	 ...
	 * 失败回调函数
	 */
	jQuery.ajaxall = function(url, data, async, type, dataType, successfn, errorfn) {
	 ...
	 */
	jQuery.ajaxs = function(url, data, successfn) {
		data = (data == null || data == "" || typeof (data) == "undefined") ? {
			"date" : new Date().getTime()
		} : data;
		$.ajax({
	 ...
	 * text/plain
	 */
	jQuery.ajaxse = function(url, data, successfn, errorfn) {
		data = (data == null || data == "" || typeof (data) == "undefined") ? {
			"date" : new Date().getTime()
		} : data;
		$.ajax({
	 ...
	 * text/plain
	 */
	jQuery.ajaxtse = function(url, data,datatype, successfn, errorfn) {
		data = (data == null || data == "" || typeof (data) == "undefined") ? {
			"date" : new Date().getTime()
		} : data;
		$.ajax({
	...
	/**
	 * ajax的参数只有一个 key=value 键值对形式
	 */
	jQuery.ajaxStr = function(url, data, successfn, errorfn) {
		$.ajax({
			type : "post",
			data : data,
			url : url,
